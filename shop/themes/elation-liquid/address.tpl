...
<form action="{$link->getPageLink('address.php', true)}" method="post" class="std" id="add_adress">
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		<p class="text">
			...
			<input type="number" id="phone" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone|escape:'htmlall':'UTF-8'}{/if}{/if}" />
		</p>
		<p class="text">
			<label for="phone_mobile">{l s='Mobile phone'}</label>
			<input type="number" id="phone_mobile" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile|escape:'htmlall':'UTF-8'}{/if}{/if}" />
		</p>
    ...
    <p class="required"><sup>*</sup> {l s='Required field'}</p><br /><br />
	</fieldset>
</form>
