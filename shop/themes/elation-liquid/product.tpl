...
<div id="primary_block" class="clearfix">
	...
	<!-- left infos-->
	<article id="eb-product-content" class="clearfix">
        <div class="eb-product-pad">
            ...
            <section id="eb-product-short-description">
                {if $product->description_short}
                    <div id="short_description_content">{$product->description_short}</div>
                {/if}
                {if $product->description}
                <div id="eb-product-more-wrap">
                	<a id="eb-product-more" href="javascript:{ldelim}{rdelim}">{l s='More details'}</a>
                </div>
                {/if}
            ...
            <form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
                ...
                <div class="product_attributes">
                    ...
                    {foreach from=$groups key=id_attribute_group item=group}
                        {if $group.attributes|@count}
                            <fieldset class="attribute_fieldset">
                                ...
                                <p class="select">
                                    <select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="eb-select" onchange="findCombination();getProductAttribute();{if $colors|@count > 0}$('#wrapResetImages').show('slow');{/if};">
                        ...
                        <div id="eb-buy-left">
                            ...
                            {else}
                                <p id="add_to_cart" class="buttons_bottom_block">
                                    <input type="submit" name="Submit" value="{l s='Add to cart'}" class="eb-add-cart" />
                                </p>
                            {/if}
                            <p id="eb-product-manu" {if !$product->manufacturer_name}style="display: none;"{/if} >
                    ...
                    <div id="eb-product-share" class="clearfix">
                        ...
                        <script type="text/javascript" src="http://s7.addthis.com/js/300/addthis_widget.js#pubid=xa-509b102028a792d5"></script>
                        <!-- AddThis Button END -->
                    </div>
                </div>
	...
	<!-- right infos-->
	<aside id="eb-product-images" class="clearfix">
    	<div class="eb-product-pad">
		<!-- product img-->
            <div id="image-block">
            	 {if $have_image}
                	{if $jqZoomEnabled}
                    <a id="zoom1" rel="position: 'inside' , showTitle: false" class="cloud-zoom" href="{$link->getImageLink($product->link_rewrite, $cover.id_image,'zoom_liquid')}">
                        <img alt="{$product->name|escape:'htmlall':'UTF-8'}" title="{$product->name|escape:'htmlall':'UTF-8'}" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_liquid')}">
                    </a>
                    {else}
                        <img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_liquid')}" id="bigpic" class="cbox-link"/>
                    {/if}
                {else}
                    <img src="{$img_prod_dir}{$lang_iso}-default-large_liquid.jpg" alt="" title="{$product->name|escape:'htmlall':'UTF-8'}" />
                {/if}
            </div>
                ...
                <ul id="thumbs_list_frame">
                    {if isset($images)}
                        ...
                        <li id="thumbnail_{$image.id_image}">
                             {if $jqZoomEnabled}
                               <a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'zoom_liquid')}" class="cloud-zoom-gallery" title="{$image.legend|htmlspecialchars}" rel="useZoom: 'zoom1', smallImage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_liquid')}'">
                                    <img src="{$link->getImageLink($product->link_rewrite, $imageIds, 'small_liquid')}" alt="{$image.legend|htmlspecialchars}"/>
                                </a>
                            {else}
                                <a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_liquid')}" rel="other-views" class="cbox {if (isset($image.cover) AND $image.cover == 1) OR (!isset($image.cover) AND $smarty.foreach.thumbnails.first)}shown{/if}" title="{$image.legend|htmlspecialchars}">
                                    <img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'small_liquid')}" alt="{$image.legend|htmlspecialchars}" />
                                </a>
                            {/if} 
                        </li>
                        {/foreach}
            ...
            <ul id="usefull_link_block">
                ...
                {if $have_image && !$jqZoomEnabled}
                	<li><span id="view_full_size" class="span_link"><a>{l s='View full size'}</a></span></li>
                {/if}
            </ul>
        </div>
	</aside>
...
<div id="more_info_block" class="clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
        ...
        {if (isset($quantity_discounts) && count($quantity_discounts) > 0)}<li><a id="more_info_tab_discount" href="#idTab11">{l s='Quantity discount'}</a></li>{/if}
...
{if isset($HOOK_PRODUCT_FOOTER) && $HOOK_PRODUCT_FOOTER}
    <div id="eb-product-footer" class="clearfix">
        {$HOOK_PRODUCT_FOOTER}
    </div>
{/if}
{/if}
