...
{if isset($lastProductAdded) AND $lastProductAdded}
	<div class="cart_last_product">
		...
		<a  class="cart_last_product_img" href="{$link->getProductLink($lastProductAdded.id_product, $lastProductAdded.link_rewrite, $lastProductAdded.category, null, null, $lastProductAdded.id_shop)|escape:'htmlall':'UTF-8'}"><img src="{$link->getImageLink($lastProductAdded.link_rewrite, $lastProductAdded.id_image, 'small_liquid')}" alt="{$lastProductAdded.name|escape:'htmlall':'UTF-8'}"/></a>
		...
		{if $use_taxes}
			{if $priceDisplay}
				<tr class="cart_total_price">
					<td colspan="3" class="txt-right">{if $display_tax_label}{l s='Total products (tax excl.):'}{else}{l s='Total products:'}{/if}</td>
					<td colspan="1" class="price" id="total_product">{displayPrice price=$total_products}</td>
				</tr>
			{else}
				<tr class="cart_total_price">
					<td colspan="3" class="txt-right">{if $display_tax_label}{l s='Total products (tax incl.):'}{else}{l s='Total products:'}{/if}</td>
					<td colspan="1" class="price" id="total_product">{displayPrice price=$total_products_wt}</td>
				</tr>
			{/if}
		{else}
			<tr class="cart_total_price">
				<td colspan="3" class="txt-right">{l s='Total products:'}</td>
				<td colspan="1" class="price" id="total_product">{displayPrice price=$total_products}</td>
			</tr>
		{/if}
			...
			<tr class="cart_total_price">
				...
				{if $voucherAllowed}
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
						<fieldset>
							...
							<p><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='OK'}" class="button_mini" /></p>
						{if $displayVouchers}
				...
				<td colspan="1" class="price total_price_container" id="total_price_container">
					<p id="total_price_head">{l s='Total:'}</p>
