		...
		<ul>
			...
			{foreach from=$cmslinks item=cmslink}
				{if $cmslink.meta_title != ''}
					<li class="item"><a href="{$cmslink.link|addslashes}">{$cmslink.meta_title|escape:'htmlall':'UTF-8'}</a></li>
				{/if}
			{/foreach}
			{if $display_stores_footer}<li class="item"><a href="{$link->getPageLink('stores.php')}">{l s='Our stores' mod='blockcms'}</a></li>{/if}
