...
{if count($categoryProducts) > 0 && $categoryProducts !== false}
	...
	<div id="productscategory_list">
		<ul {if count($categoryProducts) > 3}style="width: {math equation="width * nbImages" width=107 nbImages=$categoryProducts|@count}px"{/if}>
			...
			<li>
				<a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'small_liquid')}" alt="{$categoryProduct.name|htmlspecialchars}" />
