			...
			for (i=0;i<jsonData.discounts.length;i++)
			{
				if (parseFloat(jsonData.discounts[i].price_float) > 0)
				{
					...
					if (jsonData.discounts[i].code.length)
						delete_link = '<span class="remove_link" style="position:static"><a class="delete_voucher" href="'+jsonData.discounts[i].link+'" title="'+delete_txt+'"></a></span>';
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block dt#cart_block_product_'+ domIdProduct ).length == 0)
				{
					...
					var content =  '<div class="eb-product-item clearfix">';
						...
						content += '<div class="eb-product-item-img"><a href="' + this.link + '" title="' + this.name + '"><img src="' + this.image + '" alt="' + this.name + '" /></a></div>';
